-- ============================================================================
-- 0019 — Notifications: generate_notifications RPC + realtime
--
--   The `notifications` table already exists (0001). This migration adds the
--   generator that scans live store data and posts alerts:
--     - low_stock     (v_inventory_status, active items below min)
--     - near_expiry   (medicine_batches within 90 days, qty > 0)
--     - expired       (medicine_batches past expiry, qty > 0)
--     - supplier_due  (purchase_orders with an outstanding balance)
--     - payment_due   (invoices with pending / partial status)
--   Alerts are deduped by (type, link) while unread, and auto-resolved to
--   read once the underlying condition clears (self-healing feed).
-- ============================================================================
...
-- Realtime: let the header bell + notifications page update live.
do $$
begin
