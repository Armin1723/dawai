-- ============================================================================
-- MediFlow AI — 0014_pos_payment_status_rounding.sql
-- Code-review fixes for 0013's create_sale:
-- 1. payment_status precedence: a fully-tendered sale must be 'paid' even when
--    a split tender arrives alongside a stale p_payment_method = 'credit'
--    (previously the credit branch ran first and stamped a fully-paid split
--    sale 'pending'). Now: amount_due = 0 → 'paid' wins, then credit → 'pending',
--    otherwise → 'partial'.
-- 2. Deterministic rounding: v_subtotal / v_tax accumulate with >2dp
--    intermediate values when a line quantity has 3dp. The strict split check
--    (v_split_sum > v_total) could then reject a valid tender by a hair.
--    Round both to 2dp before computing v_total so client and server agree
--    to the paisa and the stored totals match the rounded invoice columns.
-- ============================================================================
...
begin
  ...
  -- Round to the paisa so stored totals and the split-tender check are exact.
  ...
  -- Sale row. Tender decides: fully paid → 'paid' (even if a split tender
  -- arrived alongside a stale 'credit' method), credit → 'pending',
  -- short tender → 'partial'.
