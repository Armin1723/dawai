-- ============================================================================
-- MediFlow AI — 0007_pos_data_integrity.sql
-- Code-review fixes for the POS sale path:
--   1. select_sale_batches: exclude already-expired stock from allocation.
--   2. create_sale: persist per-item discounts on sale_items (proportional
--      share per allocation chunk) and compute gst/line_total on the
--      discounted net.
--   3. create_sale: enforce customer credit limit (0 = unlimited).
--   4. create_sale: clamp total at 0 (no negative sale rows).
--   5. create_sale: lock batch rows (FOR UPDATE) during pass 1 to prevent
--      concurrent checkouts overselling into negative stock.
--   6. create_sale: non-credit methods with no amount received default to
--      full payment so invoice.status stays consistent with sales.payment_status.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. FEFO/FIFO allocation must never sell expired stock.
-- ----------------------------------------------------------------------------
create or replace function public.select_sale_batches(
  p_medicine_id uuid,
  p_quantity numeric,
...
begin
  ...
  with ranked as (
    ...
    where b.medicine_id = p_medicine_id
      and b.quantity > 0
      and b.expiry_date > current_date   -- never allocate expired stock
  )
...
-- ----------------------------------------------------------------------------
-- 2–6. create_sale with the integrity fixes.
-- ----------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    loop
      -- Proportional share of the line discount for this allocation chunk.
